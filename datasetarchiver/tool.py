...
def get_checksums(dataset_path: Path) -> Dict[str, Dict['str', 'str']]:
    """
    Calculated for all files in the dataset_path (except the meta.json) the md5 hash and returns a dict of the form
    {
    ...
    return {
        str(PurePosixPath(file.relative_to(dataset_path))): md5(file)
        for file in files
    }
...
def check_checksums(checksums_a: Dict[str, str], checksums_b: Dict[str, str]) -> Dict[str, Tuple[str, str]]:
    """
    ...
    for all checksums that differ. If a file does only exits in one dict, checksum is None.
    :param checksums_a:
    :param checksums_b:
    :return:
    """
    ...
    return {
        ...
        for key in keys
        if
        checksums_a.get(key, None) != checksums_b.get(key, None) or (key not in checksums_a and key not in checksums_b)
    }
...
def main(args):
    parser = argparse.ArgumentParser(
        ...
        'The resulting dataset archive will contain a meta.json containing a unique md5 checksum of the dataset as well as other useful inforamtion'
    )
    sub_parsers = parser.add_subparsers(dest='command')

    update_parser = sub_parsers.add_parser('create',
                                           help='Creates a archive containing the dataset and the meta data')

    update_parser.add_argument('--dataset_dir', type=is_valid_file, required=True, help='Directory of the dataset')
    update_parser.add_argument('--meta_data', type=str, default=None, required=False,
                               help='A string following the json format, used to updated the values of the meta.json.')

    check_parser = sub_parsers.add_parser('check',
                                          help='Checks if two dataset are the same (according to the meta data)')
